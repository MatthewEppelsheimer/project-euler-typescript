...
function getFactorsOf(
  ...
  while (factorCandidate > 1) {
    ...
    if (
      -1 === factors.indexOf(factorCandidate) &&
      0 === product % factorCandidate
    ) {
...
function testGetFactorsOf(): void {
...

  {
    ...
    test<typeof getFactorsOf>(
      `uses cached factors of factors`,
      () => {
