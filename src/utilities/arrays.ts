function filterFnFactoryMaxValue(maxValue: number): (value: number, index: number, array: number[]) => boolean {
    return (value: number, _i: number, _a: number[]): boolean => value <= maxValue;
}
